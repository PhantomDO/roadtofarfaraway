...
public class MapVisualizer : MonoBehaviour
{
    ...
    private readonly Dictionary<Vector3, GameObject> _obstacles = new Dictionary<Vector3, GameObject>();
    ...
    private void VisualizeUsingPrefabs(MapGrid grid, MapData data)
    {
        ...
        // Setting path roads
        foreach (var position in data.path)
        {
    ...
    private void VisualizeUsingPrimitives(MapGrid grid, MapData data)
    {
        ...
        for (int i = 0; i < data.obstacleArray.Length; i++)
        {
            if (data.obstacleArray[i])
            {
                ...
                if (_obstacles.ContainsKey(position) == false)
                {
    ...
    private void CreateIndicator(Vector3 position, Color color, PrimitiveType primitiveType)
    {
        ...
        _obstacles.Add(position, element);
        element.transform.position = position + new Vector3(0.5f, 0.5f, 0.5f);
        element.transform.parent = _parent;
        Renderer rendererComponent = element.GetComponent<Renderer>();
        rendererComponent.material.SetColor(Color1, color);
        if (animate)
        {
